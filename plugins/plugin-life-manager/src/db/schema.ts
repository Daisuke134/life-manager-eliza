...
export const goalsTable = lifeManagerSchema.table(
  "goals",
  {
...
export const planGraphsTable = lifeManagerSchema.table(
  "plan_graphs",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id").notNull(),
...
export const workItemsTable = lifeManagerSchema.table(
  "work_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id").notNull(),
...
export const decisionReceiptsTable = lifeManagerSchema.table(
  "decision_receipts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id").notNull(),
  ...
  (table) => ({
    ...
    scopeWorkItemUnique: unique(
      "lm_decision_receipts_scope_work_item_unique",
    ).on(table.agentId, table.entityId, table.workItemId),
...
export const effectIntentsTable = lifeManagerSchema.table(
  "effect_intents",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id").notNull(),
  ...
  (table) => ({
    ...
    attemptNonNegative: check(
      "lm_effect_intents_attempt_nonnegative",
      sql`${table.attempt} >= 0`,
    ),
    leaseCoherent: check(
...
export const outcomeReceiptsTable = lifeManagerSchema.table(
  "outcome_receipts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id").notNull(),
