...
- [ ] Build the immutable whitelist projection; compute unrealised P&L from positions and total P&L from `$100,000` starting equity.
- [ ] Verify serialized output contains no supplied private sentinel and preserves the judge-visible campaign lifecycle.
...
- [ ] Add `GET /api/life-manager/alpaca/public`.
- [ ] Read the official CLI campaign snapshot and latest scoped decision/effect/outcome rows.
- [ ] Return `200` projection or `503` with a generic read-only error; never return raw provider/model output.

### Task 3: Focused verification and integration

- [ ] Run `bun run --cwd plugins/plugin-life-manager typecheck`.
- [ ] Run `bun run --cwd plugins/plugin-life-manager test -- alpaca-public-projection.test.ts`.
- [ ] Run `bun run --cwd plugins/plugin-life-manager build` and `git diff --check`.
- [ ] Commit, push, merge to `main`, then update the Life Manager spec without advancing A12 to done until a logged-out hosted URL is read back.
